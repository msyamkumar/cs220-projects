...
If you are feeling brave, take a look at `gen_csv.ipynb` to see how this file `pokemon_stats.csv` was generated. Don't worry if it doesn't make much sense at this point. By the end of CS220, you will be able to understand everything that is happening there. For now, just try to figure out what is happening in each of the cells.
...
def same_types(pkmn1, pkmn2):
    if project.get_type1(pkmn1) == project.get_type1(pkmn2) and project.get_type2(pkmn1) == project.get_type2(pkmn2):
        ...
        return False
```
So far, the function looks fairly straight forward, but there is something wrong with it. To see it, find the output of the following cell:
